...
namespace SystemAdmin.Repository.SystemBasicMgmt.UserSettings
{
    public class UserFormRepository
    {
        ...
        public async Task<List<UserFormViewTreeDto>> GetUserFormViewTree(long userId)
        {
            ...
            var userForm = new List<UserFormViewTreeDto>();
            // 组装树形结构
            foreach (var group in formGroup)
            {
                var getFormType = formType.Where(formgrouptype => formgrouptype.ParentId == group.FormGroupTypeId);
                userForm.Add(new UserFormViewTreeDto
                {
                    ...
                    FormTypeChildren = getFormType.ToList()
                });
            }
            return userForm;
        }
